...
import { calculateRiskScore } from '../utils/risk-engine';
...
router.post('/login', async (req, res) => {
    try {
        ...
        const history = historyEntries.map((h: any) => ({
        ...
        const riskResult = calculateRiskScore(currentFactors, history);
        let status = 'success';
        if (riskResult.level === 'High') status = 'blocked';
        else if (riskResult.level === 'Medium') status = 'flagged';
