...
router.get('/stats', async (req, res) => {
    try {
        ...
        for (const group of riskGroups) {
            ...
            if (level === 'low')
                riskDistribution.low = group._count;
            else if (level === 'medium')
                riskDistribution.medium = group._count;
            else if (level === 'high')
                riskDistribution.high = group._count;
        }
...
router.get('/users', async (req, res) => {
    try {
        ...
        const users = await prisma.user.findMany({
            where,
            include: {
                loginHistory: { orderBy: { timestamp: 'desc' }, take: 1 },
...
router.get('/users/:id', async (req, res) => {
    try {
        ...
        res.json({
            ...
            failedAttempts: user.failedAttempts.map((f) => ({
                id: f.id,
